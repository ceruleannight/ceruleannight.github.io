...
var WorldGrid = (function () {
    ...
    //GetCellCount()
    //{
    //	return this.cellsH*this.cellsV;
    //}
    //************************************************
    ...
    WorldGrid.prototype.Render = function () {
        ...
        for (var y = 0; y < this.cellsV; y++) {
            for (var x = 0; x < this.cellsH; x++) {
                ...
                bonesArr.push(RenderImg("bone-" + idx, x * cs, y * cs, cs, cs, "Images/Crossbones_9x9.gif"));
                idx++;
            }
        }
        //document.getElementById("gridDiv").innerHTML=gridArr.join("")+entitiesArr.join("")+bonesArr.join("");
        document.getElementById("gridDiv").innerHTML = gridArr.concat(entitiesArr, bonesArr).join("");
    };
    //************************************************
...
function RenderCell(id, x, y, w, h, content, cssClass) {
    ...
    //if(g_debugMode)
    //{
    //	var xy=new IdxToXy(idx);
    //	html+=" onmouseover=\"WriteTestInfo('Cell: "+xy.x+","+xy.y+" (IDX:"+idx+")');\"";
    //	html+=" onmouseout=\"WriteTestInfo('');\"";
    //}
    html += ">" + content + "</div>";
    return html;
}
...
function PaintCell(x_gridPixel, y_gridPixel) {
    var pos = g_world.grid.GridPixelToCellPos(x_gridPixel, y_gridPixel);
    g_world.SpawnOrInfluenceCell(pos.x, pos.y, "FF0000");
}
//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\//\\
